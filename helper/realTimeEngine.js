        ...
        socket.on('registerAPMMandiri', async (asd) => {
            ...
            try {
                ...
                if(cekBooking.length>0){
                    ...
                    let hasil = await antrian_list.create({ id: uuid_v4(), tanggal_antrian: tgl, is_master: 1, poli_layanan: 1, initial, antrian_no:antrian_no[0].total, sequence: sequence_no[0].total,booking_id,jadwal_dokter_id,poli_id:idPoli,master_loket_id })
                    hasil.dataValues.sisa_antrian = +sisa[0].total
